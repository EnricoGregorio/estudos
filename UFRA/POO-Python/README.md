...
* **Aula 10 - Classes Abstratas e Sobrecargas de Operadores:** Aprenda sobre **calsses abstratras**, o porquê ela não pode ser instanciada e quando vale a pena criar uma (`ABC` e `@abstractmethod`). Além disso, foi visto a diferença entre **identidade** (`is`) e **igualdade** (`==`), e como mudar a regra do `==` com `__eq__`. Por fim, veja sobre **sobrecarga de operadores** pelos *métodos mágicos*: `__str__`, `__repr__`, `__eq__`, `__hash__` e `__lt__`. Esses temas fecham o pilar da **abstração** com vocabulário formal e dão sequência ao **polimorfismo** da Aula 6, agora na forma de operadores que respondem que respondem aos nossos objetos.
* **Aula 11 - Type hints e dataclasses:** Revisa a diferença entre **tipagem dinâmica e estática**, e onde Python se encaixa. Além disso, **Type hints** também são visitados novamente, com a intenção de anotar parâmetros e retornos, com `Optional` (objeto ou nada) e `Union` (um tipo ou outro). É visto o que é o decorator `@dataclass` e o código que ele escreve por você; quando uma *dataclass* basta e quando vale uma classe feita à mão. Por fim, é explorado o conceito de **imutabilidade** com `frozen=True`, e o cuidado com valores *default* mutáveis (`default_factory`). São recursos da linguagem que apoiam o paradigma OO: contratos de tipo e classes de dados curtas.
